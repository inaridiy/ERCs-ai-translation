---
original: d6ca3c0c908c7267385595e6e3e30aea0d8c74ea5ff8c86ae62ca4bb5b7336da
---
...
特権アクションのアクセス制御を実装する方法は多数あります。一般的に使用されるパターンは[ERC-5982](./eip-5982.md)で指定された「ロールベース」のアクセス制御です。しかし、この方法では役割の付与や取り消しをインターフェイスを通して行うためのオフチェーンの管理ツールが必要になります。さらに、多くのウォレットにはユーザーに付与された特権を表示する機能がないため、ユーザーはウォレットからその特権状況を把握することができません。

### ユースケース

このERCは、[ERC-5982](./eip-5982.md)で説明されているロールベースのアクセス制御が使用されるシナリオで適用できます。具体的なユースケースは以下のとおりです:

**ミント/バーン権限:**
チケット、クーポン、会員カード、サイトアクセス権などのアイテムをトークンとして流通させるアプリケーションでは、システム管理者にこれらのトークンをミントまたはバーンする権限を与える必要があります。これらの権限は、このスキームのコントロールトークンとして実現できます。

**転送権限:**
これらのアプリケーション内の一部のシナリオでは、トークンの転送先を特定の機関に制限したい場合があります。そのような場合、機関証明書をコントロールトークンとして発行し、このトークンの所有権によってトークンの転送を管理することができます。

**アドレス検証:**
多くのアプリケーションでは、ミントや転送時の宛先アドレスの誤りを防ぐためにアドレス検証が必要です。ユーザーにアドレス検証の証明としてコントロールトークンが発行され、ミントや転送時にこのトークンの所有が要求されることで、誤配送を防ぐことができます。この場合、政府機関や特定の企業によってアイデンティティ検証プロセス後にコントロールトークンが発行される可能性があります。
...
3. アカウントが必要な役割を持っているかどうかを確認するには、ERC-721またはERC-1155で定義された`balanceOf`メソッドを使用して、コントロールトークンの残高が0より大きいことを確認すべきです。ERC-1155トークンの場合は、`typeId`を指定する必要があります。
4. 役割をアカウントに付与するには、特権を表すコントロールトークンを[ERC-5679](./eip-5679.md)で定義された`safeMint`メソッドを使ってアカウントにミントすべきです。
5. アカウントから役割を取り消すには、特権を表すコントロールトークンをERC-5679の`burn`メソッドを使ってバーンすべきです。
6. 準拠するスマートコントラクト内の役割は`bytes32`形式で表現されます。役割名の文字列をkeccak256ハッシュした値を使うことが推奨されます。例: `bytes32 role = keccak256("MINTER")`.

## 根拠

ERC-721またはERC-1155トークンをコントロールトークンとして使用することで、ユーザーがウォレット内で特権を視覚的に確認できるようになり、特権管理が簡単になります。

一般的に、特権をトークン化する際には、Soulbound Token(例: [ERC-5192](./eip-5192.md))のような仕様が使用されます。ERC-5192はERC-721を継承しているため、このERCではコントロールトークンとしてERC-721を選択しています。
...
ERC-7303は、アプリケーションでTCTC型のアクセス制御を実装するためのモディファイアを提供しています。このモディファイアは、アカウントが必要な役割を持っているかどうかをチェックします。また、特定の役割をアカウントに付与する関数も含まれています。
...
abstract contract ERC7303 {
     ...
     * 複数回呼び出すことができ、その場合ユーザーは指定されたトークンのいずれかを所有している必要がある。
     * @param role byte32 付与する役割。
     * @param contractId address コントロールトークンのコントラクトID。
     */
     ...
     * @param role byte32 付与する役割。
     * @param contractId address コントロールトークンのコントラクトID。
     * @param typeId uint256 コントロールトークンのタイプID。
     */
...
以下は、ERC-721トークンにおいて「ミンター」と「バーナー」の役割を定義するために`ERC7303`を使用する簡単な例です。これらの役割を持つアカウントは、ERC-721またはERC-1155のコントロールトークンを指定することで、新しいトークンの作成と既存トークンの破棄が許可されます:
...
流通対象のトークンのセキュリティは、コントロールトークンのセキュリティに大きく依存します。管理特権、ミント/転送/バーン権限、およびコントロールトークンの契約更新の可能性について慎重に検討する必要があります。

特に、コントロールトークンを転送可能にすることで、管理権限の一時的な委任などの柔軟な操作が可能になります。しかし、同時にトークン流通の権限が不適切な第三者の手に渡る可能性も高まります。したがって、一般的にコントロールトークンは非転送可能にすべきです。コントロールトークンを転送可能にする場合でも、少なくともこれらのトークンを焼却する権限は信頼できる管理者が保持するべきです。
